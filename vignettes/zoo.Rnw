...
\begin{figure}[tbhp]
 \includegraphics[totalheight=8cm]{./figs/Iggy_Azalia.png}
 \caption{The Gini variable importances for a set of data constructed so that there is a set of Null variables and a
   smaller set of non-Null variables (blue curve). The red curve shows the Gini importances for a set of the Null
   variables. This is the distribution that a permutation of the outcomes will approximate.
   The assumption behind permuting the outcomes is that the red and blue curves will overlap.
  }
  \label{figure:null_distn_of_variable_importances.png}
\end{figure}
